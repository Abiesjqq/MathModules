...
double calculateCost(candidate candidate1) {
    ...
    while (accomplished < total_tasks) {
        ...
        std::vector<double> directions = swarm(current_uavs, candidate1, accomplished);
...
bool doesSegmentIntersectCircle(const position_coord &A, const position_coord &B, const Circle &Z) {
    double xc = Z.x;
    double yc = Z.y;
    double r = Z.r;
...
//distance的辅助函数：取切点
std::vector<position_coord> getTangentPoints(const position_coord &P, const Circle &Z) {
    double xc = Z.x;
    double yc = Z.y;
    double r = Z.r;
    double px = P.x;
...
//distance的辅助函数：求弧长
double arcLength(const position_coord &C, const position_coord &D, const Circle &Z) {
    double xc = Z.x;
    double yc = Z.y;
    double dx1 = C.x - xc;
    double dy1 = C.y - yc;
        ...
        if (doesSegmentIntersectCircle(a, b, obstacle)) {
            ...
            for (const auto& c : A_tangents) {
                for (const auto& d : B_tangents) {
                    ...
                    double total = distance(a, c) + current_arc + distance(d, b);
                    if (total < min_total) {
                        min_total = total;
                        c_best = c;
                        d_best = d;
...
vector<double> swarm(
    ...
    for (int i = 0; i < num_particles; ++i) {
        particles.emplace_back(movable_drones_count); // 使用可移动无人机数量作为维度
    }
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        for (auto& p : particles) {
            ...
            // 调用适应度函数计算当前粒子的适应度
            double fitness = calculateFitness(new_uavs, candidate1, accomplished);
        ...
        for (auto& p : particles) {
            for (int i = 0; i < movable_drones_count; ++i) {
                ...
                while (p.position[i] < 0) p.position[i] += 2 * M_PI;
                while (p.position[i] >= 2 * M_PI) p.position[i] -= 2 * M_PI;
            }
        }
    }
...
double calculateFitness(
    ...
    for (int uav_id = 1; uav_id <= drone_num; ++uav_id) {
        ...
        for (const auto& assignment : candidate1.assignment) {
            if (assignment.first == uav_id && assignment.second > accomplished) {
                if (next_task_order == -1 || assignment.second < next_task_order) {
    ...
    if (accomplished < target_num) {
        ...
        if (target_uav_id != -1) {
            ...
            position_coord global_task_pos = {global_next_task.x, global_next_task.y};
...
bool isStateLegal(const vector<UAV>& uavs, int accomplished, const candidate& candidate1) {
        /**
     * @brief 判断无人机状态是否合法
     * 
     * 检查以下5种不合法情况：
    ...
    for (const UAV& uav : uavs) {
        if (uav.working_time > uav.max_working_time) return false;
    }
    ...
    if (accomplished < target_num) {
        ...
        for (size_t uav_idx = 0; uav_idx < uavs.size(); ++uav_idx) {
            ...
            if (current_uav_id == assigned_uav_id) continue;  // 跳过负责无人机
